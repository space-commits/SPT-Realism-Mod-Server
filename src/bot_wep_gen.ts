...
export class BotWepGen extends BotWeaponGenerator {
    ...
    //     const _checkRequired = new CheckRequired();
    //     const src = "IsWepValid"
    //     // this.logger.success("===+++++++++++++++++++====IS WEAPON VALID===+++++++++++++++++===");
    //     for (const item of itemList) {
    //         const template = this.databaseServer.getTables().templates.items[item._tpl];
    //         if (!template._props.Slots || !template._props.Slots.length) {
    //             continue;
    ...
    //         for (const slot of template._props.Slots) {
    //             if (!_checkRequired.checkRequired(slot, this.logger, src)) {
    //                 continue;
    //             }
...
export class BotModGen extends BotGeneratorHelper {
    ...
    // public botModGen(items: Item[], modPool: Mods, parentId: string, parentTemplate: ITemplateItem, modSpawnChances: ModsChances, isPmc = false): Item[] {
    ...
    //         const botConf = this.configServer.getConfig<IBotConfig>(ConfigTypes.BOT);
    //         const _checkRequired = new CheckRequired();
    //         const src = "Bot Mod Gen"
    //         // this.logger.success("|||||||||||||||BOT MOD GEN|||||||||||||||");

    //         const itemModPool = modPool[parentTemplate._id] ? modPool[parentTemplate._id] : {};
    //         let missingRequiredMods = parentTemplate._props.Slots.find(i => _checkRequired.checkRequired(i, this.logger, src) == true && !itemModPool[i._name])

    //         while (missingRequiredMods) {
    //             itemModPool[missingRequiredMods._name] = []
    //             missingRequiredMods = parentTemplate._props.Slots.find(i => _checkRequired.checkRequired(i, this.logger, src) == true && !itemModPool[i._name])
    //         }
    ...
    //             if (!this.shouldModBeSpawned(itemSlot, modSlot, modSpawnChances)) {
    //                 continue;
    //             }

    //             const ammoContainers = ["mod_magazine", "patron_in_weapon", "patron_in_weapon_000", "patron_in_weapon_001", "cartridges"];
    //             const modSpawnChance = _checkRequired.checkRequired(itemSlot, this.logger, src) || ammoContainers.includes(modSlot) ? 100 : modSpawnChances[modSlot];
    ...
    //                 const cartridgeBlacklist = botConf.pmc.cartridgeBlacklist;
    ...
    //             // Find a mod to attach from items db for required slots if none found above
    //             const parentSlot = parentTemplate._props.Slots.find(i => i._name === modSlot);
    //             if (!found && parentSlot !== undefined && _checkRequired.checkRequired(parentSlot, this.logger, src) == true) {
    //                 modTpl = this.getModTplFromItemDb(modTpl, parentSlot, modSlot, items);
    //                 found = !!modTpl;
    //             }

    //             if (!found || !modTpl) {
    //                 if (_checkRequired.checkRequired(itemSlot, this.logger, src)) {
    //                     this.logger.error(`Could not locate any compatible items to fill '${modSlot}' for ${parentTemplate._id}`);
    //                 }
    //                 continue;
    //             }
    ...
    //             }
    //             else {
    //                 if (Object.keys(modPool).includes(modTpl) || (modTemplate._props.Slots && modTemplate._props.Slots.find(i => _checkRequired.checkRequired(i, this.logger, src) == true))) {
    ...
    // public botWepGen(items: Item[], modPool: Mods, parentId: string, parentTemplate: ITemplateItem, modSpawnChances: ModsChances, isPmc = false): Item[] {
    //     {
    //         const itemModPool = modPool[parentTemplate._id];
    //         const _checkRequired = new CheckRequired();
    ...
    //             const ammoContainers = ["mod_magazine", "patron_in_weapon", "patron_in_weapon_000", "patron_in_weapon_001", "cartridges"];
    //             const modSpawnChance = _checkRequired.checkRequired(itemSlot, this.logger) || ammoContainers.includes(modSlot) ? 100 : modSpawnChances[modSlot];

    //             // Filter blacklisted cartridges
    //             if (isPmc && this.getAmmoContainers().includes(modSlot)) {
    //                 itemModPool[modSlot] = this.getNonBlacklistedCartridges(modSlot, itemModPool);
    //             }
    ...
    //             // Find a mod to attach from items db for required slots if none found above
    //             const parentSlot = parentTemplate._props.Slots.find(i => i._name === modSlot);
    //             if (!found && parentSlot !== undefined && _checkRequired.checkRequired(parentSlot, this.logger)) {
    //                 modTpl = this.getModTplFromItemDb(modTpl, parentSlot, modSlot, items);
    //                 found = !!modTpl;
    //             }

    //             if (!found || !modTpl) {
    //                 if (_checkRequired.checkRequired(itemSlot, this.logger)) {
    //                     this.logger.error(`Could not locate any compatible items to fill '${modSlot}' for ${parentTemplate._id}`);
    //                 }
    //                 continue;
    //             }
